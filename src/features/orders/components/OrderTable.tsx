...
export function OrderTable({ orders, onSelectOrder, onCancelOrder }: OrderTableProps) {
  ...
  const filteredOrders = useMemo(() => {
    return orders
      .filter(o => 
        o.instrument.toLowerCase().includes(filters.instrument.toLowerCase()) &&
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-lg border dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700" aria-label="Trading orders table">
          <thead className="bg-gray-50 dark:bg-gray-900">
            <tr>
              <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">ID</th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Instrument</th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Side</th>
              <th 
                scope="col" 
              ...
              <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Rem. Qty</th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
              <th 
                scope="col" 
                className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer select-none" 
                onClick={() => toggleSort('createdAt')}
                aria-sort={sortKey === 'createdAt' ? (sortOrder === 'asc' ? 'ascending' : 'descending') : 'none'}
              >
            ...
            {paginatedOrders.length === 0 ? (
              ...
              paginatedOrders.map(order => (
                <tr 
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onSelectOrder(order.id); }}
                >
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="page-size" className="text-sm text-gray-500">Page size:</label>
            <select 
              id="page-size"
              className="p-1 border rounded text-sm dark:bg-gray-700 dark:border-gray-600 outline-none"
              value={pageSize}
